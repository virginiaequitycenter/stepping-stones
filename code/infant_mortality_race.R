...
# pdf20area <- list(setNames(c(76, 35, 760, 590), 
...
#   select(locality, year, numdeaths:ratedeaths_other)
# 
...
extract_appvdh <- function(pdfurl, year, localities) {
  ...
  list <- extract_tables(pdfurl, area = pdfarea, pages = 1,
                           guess = FALSE, output = "data.frame")
  dflist <- list[[1]]
  names(dflist) <- c("locality", "numdeaths1", "numdeaths1_white", 
                   "numdeaths1_black", "numdeaths1_other",
                   "numdeaths", "numdeaths_white", "numdeaths_black",
                   "numdeaths_other", "ratedeaths", "ratedeaths_white",
                   "ratedeaths_black", "ratedeaths_other")
...
infmort_list <- map(infmort_list, ~mutate_all(., as.character))
infmort <- bind_rows(infmort_list)
infmort <- infmort %>% 
  mutate(year = as.numeric(year),
         across(contains("num"), as.integer),
...
inf_mort_allyears <- bind_rows(infmort, infmortb,
  ...
  mutate(across(numdeaths:ratedeaths_other, ~replace_na(.x, 0)))
...
inf_mort_allyears <- inf_mort_allyears %>% 
  ...
  group_by(locality) %>% 
  mutate(rate_all_3yr = zoo::rollmean(ratedeaths, k = 3, fill = NA, align = "center"),
         rate_white_3yr = zoo::rollmean(ratedeaths_white, k = 3, fill = NA, align = "center"),
         rate_black_3yr = zoo::rollmean(ratedeaths_black, k = 3, fill = NA, align = "center"),
