...
#   annotate("text", x = 6.2, y = 55, size = 3, hjust = 0, color = "#333333", 
#            label = 
...
(2007-2009)") +
  ...
  geom_text_repel(data = per_cap_income_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1.8, show.legend = FALSE, min.segment.length = Inf) +
  scale_linetype_manual(values = c(2, 1)) +
...
child_poverty %>% 
  ...
  annotate("text", x = 16.8, y = 85, size = 3, hjust = 0, color = "#333333", 
           label = 
"COVID-19 Economic
Stimulus Payments") +
  geom_line() +
...
economically_disadvantaged_students %>%
  ...
  geom_vline(xintercept = "2018-2019", linetype = 2, color = "#333333") +
  annotate("text", x = 15.1, y = 70, size = 3, hjust = 0, color = "#333333", 
           label = 
"CCS begin 
participating 
in USDA CEP") +
  geom_vline(xintercept = "2021-2022", linetype = 2, color = "#333333") +
  annotate("text", x = 18.1, y = 10, size = 3, hjust = 0, color = "#333333", 
           label = 
"ACPS begin 
participating 
in USDA CEP") +
...
rent_burdened <- rent_burdened %>%
  mutate(locality = str_remove(NAME, " County, Virginia| city, Virginia"), 
         locality = str_trim(locality),
         survey_period = paste0(as.character(year-5), "-", as.character(year))) 
rent_burdened_labels <- rent_burdened %>% 
  filter(locality != "Virginia" & survey_period == "2016-2021") %>%
  mutate(label = paste0(as.character(round((percent_over_30*100), digits = 1)), "%"))

rent_burdened %>% 
  ggplot(aes(x = survey_period, y = percent_over_30, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = rent_burdened_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 1),
                     labels = scales::percent, 
...
homeowner_burden <- homeowner_burden %>%
  mutate(locality = str_remove(NAME, " County, Virginia| city, Virginia"), 
         locality = str_trim(locality),
         survey_period = paste0(as.character(year-5), "-", as.character(year)))
homeowner_burden_labels <- homeowner_burden %>% 
  filter(locality != "Virginia" & survey_period == "2016-2021") %>%
  mutate(label = paste0(as.character(round((pct_burden), digits = 1)), "%"))

homeowner_burden %>% 
  ggplot(aes(x = survey_period, y = pct_burden, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = homeowner_burden_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 100),
                     labels = label_percent(scale = 1), 
...
pitcount_homeless <- pitcount_homeless %>% 
  mutate(year = as.character(year),
         ...
         type = recode(type,
                       ...
                       "total_persons" = "Total"))
pitcount_homeless_labels <- pitcount_homeless %>% 
  filter(year == "2022") %>%
  mutate(label = paste0(as.character(count)))
...
pitcount_homeless %>% 
  ggplot(aes(x = year, y = count, 
             ...
             linetype = type)) +
  geom_vline(xintercept = "2012", linetype = 2, color = "#333333") +
  annotate("text", x = 4.2, y = 100, size = 3, hjust = 0, color = "#333333", 
           label = 
"Opening of The Crossings
(permanent supportive housing)") +
  geom_vline(xintercept = "2022", linetype = 2, color = "#333333") +
  annotate("text", x = 10.2, y = 275, size = 3, hjust = 0, color = "#333333", 
           label = 
...
and lack of affordable housing") +
  geom_line() +
  geom_point() +
  geom_text_repel(data = pitcount_homeless_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .8, show.legend = FALSE, min.segment.length = Inf) +
...
### Access to Prenatal Care {.tabset}
...
prenatal_first_trimester <- prenatal_first_trimester %>%
  mutate(percent_access_to_prenatal_care = 100 - late_no_prenatal_to_birthrates,
          year = as.character(year))
prenatal_first_trimester_labels <- prenatal_first_trimester %>% 
  filter(locality != "Virginia" & year == "2020") %>%
  mutate(label = paste0(as.character(round((percent_access_to_prenatal_care), digits = 1)), "%"))
...
prenatal_first_trimester %>%
  ...
  annotate("text", x = 5.2, y = 80, size = 3, hjust = 0, color = "#333333", 
           label = 
"Affordable Care 
Act (ACA) passed") +
  geom_vline(xintercept = "2019", linetype = 2, color = "#333333") +
  annotate("text", x = 11.5, y = 85, size = 3, hjust = 0, color = "#333333", 
           label = 
"Virginia expanded 
Medicaid coverage") +
  geom_line() +
  geom_point() +
  geom_text_repel(data = prenatal_first_trimester_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1.2, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(40, 100),
                     labels = label_percent(scale = 1), 
...
low_birth_weight_labels <- low_birth_weight %>% 
  filter(locality != "Virginia" & three_yr_period == "2018-2020") %>%
  mutate(label = paste0(as.character(round((pct_3yr), digits = 1)), "%"))

low_birth_weight %>%
  ggplot(aes(x = three_yr_period, y = pct_3yr, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = low_birth_weight_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1.2, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 60),
                     labels = label_percent(scale = 1), 
...
infant_mortality <- infant_mortality %>%
  ...
  mutate(locality = recode(locality,
                       ...
                       "Commonwealth Of Virginia" = "Virginia"),
         three_yr_period = paste0(as.character(year-1),"-",as.character(year+1)))
infant_mortality_labels <- infant_mortality %>% 
  filter(locality != "Virginia" & three_yr_period == "2018-2020") %>%
  mutate(label = paste0(as.character(round((rate_3yr), digits = 1)), "%"))

infant_mortality %>% 
  ggplot(aes(x = three_yr_period, y = rate_3yr, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = infant_mortality_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1.2, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 60),
                     labels = label_percent(scale = 1), 
...
teen_pregnancy_birth <- teen_pregnancy_birth %>%
  ...
  mutate(locality = recode(locality,
                          ...
                          "Commonwealth Of Virginia" = "Virginia"),
         three_yr_period = paste0(as.character(year-1),"-",as.character(year+1)))
teen_pregnancy_labels <- teen_pregnancy_birth %>% 
  filter(locality != "Virginia" & three_yr_period == "2018-2020") %>%
  mutate(label = as.character(round((pregnancyrate_3yr), digits = 1)))

teen_pregnancy_birth %>% 
  ggplot(aes(x = three_yr_period, y = pregnancyrate_3yr, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = teen_pregnancy_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 100), 
...
```{r}
teen_birth_labels <- teen_pregnancy_birth %>% 
  filter(locality != "Virginia" & three_yr_period == "2018-2020") %>%
  mutate(label = as.character(round((birthsrate_3yr), digits = 1)))

teen_pregnancy_birth %>% 
  ggplot(aes(x = three_yr_period, y = birthsrate_3yr, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = teen_birth_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .8, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 100), 
                     name = "Rate per 1,000 Teens (15-17)") + 
...
youth_sti_labels <- youth_sti %>% 
  filter(locality != "Virginia" & year3 == "2019-2021") %>%
  mutate(label = as.character(round((total_incrate_3yr), digits = 1)))

youth_sti %>%
  ggplot(aes(x = year3, y = total_incrate_3yr, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = youth_sti_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .8, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 100),
...
children_twoparents_labels <- children_twoparents %>% 
  filter(locality != "Virginia" & survey_period == "2016-2021") %>%
  mutate(label = paste0(as.character(round((percent*100), digits = 1)), "%"))

children_twoparents %>% 
  ggplot(aes(x = survey_period, y = percent, color = locality, group = locality)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = children_twoparents_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .9, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 1),
                     labels = scales::percent, 
                     name = "Percent of Children") + 
...
divorce_vdh <- divorce_vdh %>%
  mutate(locality = recode(locality,
                          ...
                          "Charlottesville City" = "Charlottesville"),
         year = as.character(year))
divorce_vdh_labels <- divorce_vdh %>% 
  filter(locality != "Virginia" & year == "2018") %>%
  mutate(label = as.character(round((divorce_rate), digits = 1)))
...
divorce_vdh %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = divorce_vdh_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .9, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
...
divorce_circuit <- divorce_circuit %>% 
  ...
  mutate(year = as.character(year))
divorce_circuit_labels <- divorce_circuit %>% 
  filter(locality != "Virginia" & year == "2022") %>%
  mutate(label = as.character(round((divorce_rate), digits = 1)))
...
divorce_circuit %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = divorce_circuit_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,10),
                     breaks = c(0,2,4,6,8,10),
...
child_welfare_reports <- child_welfare_reports %>%
  mutate(fiscal_year = paste0(as.character(year-1),"-",as.character(year)),
         year = as.character(year))
child_welfare_reports_labels <- child_welfare_reports %>% 
  filter(locality != "Virginia" & fiscal_year == "2019-2020") %>%
  mutate(label = as.character(round((cps_rate), digits = 1)))
...
child_welfare_reports %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = child_welfare_reports_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,100),
...
foster_care <- foster_care %>%
  mutate(year = as.character(year))
foster_care_labels <- foster_care %>% 
  filter(locality != "Virginia" & year == "2022") %>%
  mutate(label = as.character(round((fc_rate), digits = 1)))
...
foster_care %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = foster_care_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .8, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,100),
                     name = "Rate per 1000 Children") +
...
##### Rate of Incidents of Possession of Alcohol and Drugs for Albemarle County Public Schools, Charlottesville City Schools, and Virginia Public Schools
```{r}
school_alcdrugs_dcv <- read_csv("../data/school_alcdrugs_dcv.csv")
school_alcdrugs_dcv_labels <- school_alcdrugs_dcv %>% 
  filter(division != "Virginia" & school_year == "2020-2021") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
school_alcdrugs_dcv %>%
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = school_alcdrugs_dcv_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
...
school_violence_dcv_labels <- school_violence_dcv %>% 
  filter(division != "Virginia" & school_year == "2020-2021") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
school_violence_dcv %>%
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = school_violence_dcv_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,100),
                     name = "Rate of Incidents per 1000 Students") +
...
school_weapons_dcv_labels <- school_weapons_dcv %>% 
  filter(division != "Virginia" & school_year == "2020-2021") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
school_weapons_dcv %>%
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = school_weapons_dcv_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,50),
                     name = "Rate of Incidents per 1000 Students") +
...
##### Rate of Students Given an Out-of-School Suspension (Short-term Suspension, Long-term Suspension, or Modified Expulsion to Suspension) for Albemarle County Public Schools, Charlottesville City Schools, and Virginia Public Schools
```{r}
school_suspensions <- read_csv("../data/school_suspensions.csv")
school_suspensions_labels <- school_suspensions %>% 
  filter(division != "Virginia" & school_year == "2020-21") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
school_suspensions %>%
  ...
  annotate("text", x = 14.2, y = 350, size = 3, hjust = 0, color = "#333333", 
           label = 
"Replicable with 
available SSIR data") +
  geom_line() +
  geom_point() +
  geom_text_repel(data = school_suspensions_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_linetype_manual(values = c(2, 1)) +
...
child_need_services <- child_need_services %>%
  mutate(year = as.character(year))
child_need_services_labels <- child_need_services %>% 
  filter(locality != "Virginia" & year == "2022") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
child_need_services %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = child_need_services_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,100),
...
juvenile_delinquency_labels <- juvenile_delinquency %>% 
  filter(locality != "Virginia" & year == "2022") %>%
  mutate(label = as.character(round((rate), digits = 1)))
...
juvenile_delinquency %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = juvenile_delinquency_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = .7, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
...
arrests_alcoholrelated <- arrests_alcoholrelated %>%
  mutate(year = as.character(year))
arrests_alcoholrelated_labels <- arrests_alcoholrelated %>% 
  filter(locality != "Virginia" & year == "2021") %>%
  mutate(label = as.character(round((arr_rate), digits = 1)))
...
arrests_alcoholrelated %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = arrests_alcoholrelated_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,50),
...
arrests_crimesagainstpersons <- arrests_crimesagainstpersons %>%
  mutate(year = as.character(year))
arrests_crimesagainstpersons_labels <- arrests_crimesagainstpersons %>% 
  filter(locality != "Virginia" & year == "2021") %>%
  mutate(label = as.character(round((arr_rate), digits = 1)))
...
arrests_crimesagainstpersons %>% 
  ...
  geom_line() +
  geom_point() +
  geom_text_repel(data = arrests_crimesagainstpersons_labels, aes(label = label),
                  size = 3, fontface = "bold", nudge_x = 1, show.legend = FALSE, min.segment.length = Inf) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0,50),
                     name = "Rate per 1000 Youth (Under 18)") +
...
ggplot(aes(x = Year, y = meanpercsus, group = Locality, shape = Race, color = Locality)) + 
  ...
  scale_shape_manual(values = c(15,16,17,7,18,8),
                     na.translate = FALSE) +
  labs(color = "", shape = "", caption = "Note: Racial groups were only included for a given year and locality when the average
       number of students enrolled was high enough such that you would expect to see at least one suspension
       if the average overall rate were true for each racial group")+
...
#### Trends
##### Enrollement in Any Post-Secondry Education
...
ggplot(aes(x = year, y = percent_any, group = locality, shape = group, color = locality)) + 
  ...
  scale_shape_manual(values = c(23,15,16,17,7,18),
                     na.translate = FALSE) +
  labs(color = "", shape = "", caption = "Note: Racial groups were only included for a given year and locality when the number of students enrolled 
  was high enough such that you would expect to see at least one student enrolled in a 4-year institution if the 
  overall rate were true for each racial group")+
  theme(plot.caption = element_text(hjust=0), legend.position="top")
