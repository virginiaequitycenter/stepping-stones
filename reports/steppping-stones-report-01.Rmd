...
pals_kinder_labels <- pals_kinder_u %>% 
  filter(location != "Virginia") %>% 
  mutate(label = paste0(as.character(round(percent, digits = 1)), "%"))

pals_kinder_u %>%
  ggplot(aes(x = school_year, y = percent, color = location, group = location, label = percent)) +
  geom_line() +
  geom_point() +
  geom_text_repel(data = pals_kinder_labels, aes(label = label), size = 2.5, max.overlaps = 15) +
...
sols_m3_labels <- sols %>% 
  filter(division_name != "Virginia" & subject == "Mathematics" & test_level == "Grade 3") %>% 
...
released by the VDOE") +
  geom_line() +
  geom_point() +
  geom_text_repel(data = sols_m3_labels, aes(label = label), size = 2.5, point.padding = 0.5, max.overlaps = 15) +
  scale_color_manual(values = three_category_palette) +
  scale_y_continuous(limits = c(0, 100),
                     labels = label_percent(scale = 1), 
                     name = "Pass Rate") + 
